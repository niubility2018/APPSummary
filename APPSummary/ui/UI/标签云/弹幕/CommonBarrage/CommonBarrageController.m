...
- (void)initBtnView{
    ...
    [pauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(stopBtn.mas_right).mas_offset(0);
        make.bottom.mas_equalTo(0);
        make.size.mas_equalTo(CGSizeMake(screen_width/6, 40));
    }];
    
    UIButton *resumeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    resumeBtn.backgroundColor = [UIColor cyanColor];
    ...
    [resumeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(pauseBtn.mas_right).mas_offset(0);
        make.bottom.mas_equalTo(0);
        make.size.mas_equalTo(CGSizeMake(screen_width/6, 40));
    }];
    
    UIButton *fasterBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    fasterBtn.backgroundColor = [UIColor cyanColor];
    [fasterBtn setTitle:@"加速" forState:UIControlStateNormal];
    [fasterBtn addTarget:self action:@selector(faster:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:fasterBtn];
    [fasterBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(resumeBtn.mas_right).mas_offset(0);
        make.bottom.mas_equalTo(0);
        make.size.mas_equalTo(CGSizeMake(screen_width/6, 40));
    }];
    
    UIButton *slowerBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    slowerBtn.backgroundColor = [UIColor cyanColor];
    [slowerBtn setTitle:@"减速" forState:UIControlStateNormal];
    [slowerBtn addTarget:self action:@selector(slower:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:slowerBtn];
    [slowerBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(fasterBtn.mas_right).mas_offset(0);
        make.bottom.mas_equalTo(0);
        make.size.mas_equalTo(CGSizeMake(screen_width/6, 40));
    }];
