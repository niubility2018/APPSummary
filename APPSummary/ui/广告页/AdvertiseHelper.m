...
{
    NSString *url = @"https://github.com/niubility2018/APPSummary";
    if (!LMJIsEmpty(url)) {
        if([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:url]]) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url] options:@{@"username" : @"niubility2018"} completionHandler:^(BOOL success) {

                NSLog(@"%zd", success);
            }];
        }
    }
//    APPQuestionDetailController *appqvc = [[APPQuestionDetailController alloc] init];
//    appqvc.urlStr = [NSURL URLWithString:@"http://xubojoy.cn"];
//    [[UIApplication sharedApplication].delegate.window.rootViewController.navigationController pushViewController:appqvc animated:YES];
}
